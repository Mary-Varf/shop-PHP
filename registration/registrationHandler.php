...
if(mysqli_connect_errno()) {
    ...
    $out['usersname'] == '' || $out['password'] == '' || $out['surname'] == '' || $out['email'] == '' || $out['phone'] == ''|| $out['passwordVer'] == '') {
        ...
        if($result && $result->num_rows > 0) {
            ...
            $line = "INSERT INTO `users` (`login`, `name`, `surname`, `phone`, `patronymic`, `password`) VALUES ('$email', '$name', '$surName', '$phone', '$thirdName', '$hash');";
            $result = mysqli_query($connect, $line);
            $insert_id = $connect->insert_id;
            $line2 = "INSERT INTO `role_user` (`users_id`, `roles_id`) VALUES ('$insert_id', '3');";
            $resultRole = mysqli_query($connect, $line2);
